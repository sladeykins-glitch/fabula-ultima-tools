...
import { applyOfficialMonsterStructure, chooseOfficialInspiredItemCandidate, createMonsterVariant, GeneratorPowerIntent, MonsterVariant, monsterCoherenceSummary, officialInspiredItemBudget, officialInspiredMonsterSettings, powerAdjustedItemBudget, powerAdjustedMonsterSettings } from './generatorEvolution'
...
import { aestraNations, aestraOrigins, applyAestraMonsterIdentity, applyAestraCrystalInfluence, aestraGenerationHint, refreshAestraAffinities, refreshAestraMonsterLabel, refreshAestraMonsterTraits, AestraInfluence, AestraNation, AestraOrigin, valdoriaDepths, ValdoriaDepth } from './aestra'
...
import { itemQualitySummary, monsterQualitySummary, validateItemDesign, validateMonsterDesign } from './generatorQuality'
import { applyAestraRegionalVisualIdentity } from './aestraVisualIdentity'
...
function buildMonsterArtPrompt(monster: Monster, preset: ArtPromptPreset, mode: ArtPromptMode, framing: ArtPromptFraming) {
  ...
  const family = monster.family ? `creature family ${monster.family.familyName}, ${monster.family.form.toLowerCase()} form, visibly related to ${monster.family.baseMonsterName}` : ''
  ...
  if (mode === 'Short') return [style,regional,frame,monster.name,identity,family,visualBlock,`traits: ${traits}`,promptSentence(appearance),`visible ${damage} power`,promptSentence(habitat),finish].filter(Boolean).join(', ')
  if (mode === 'Variant') return [style,regional,frame,`monster concept art of ${monster.name}`,identity,family,visualBlock,description,appearance,`behaviour: ${promptSentence(behaviour)}`,`ecology: ${promptSentence(ecology)}`,`memorable detail: ${promptSentence(quirk || hook)}`,`visible ${damage} energy`,finish].filter(Boolean).join(', ')
  if (mode === 'Permutation') return [style,regional,`${monster.name}, ${identity}`,family,visualBlock,'{full-body bestiary portrait, dramatic three-quarter pose, atmospheric environment portrait, dynamic battle-ready stance}',`traits: ${traits}`,promptSentence(appearance),`visible ${damage} power`,promptSentence(habitat),finish].filter(Boolean).join(', ')
  return [style,regional,frame,`bestiary illustration of ${monster.name}`,`${identity}, level ${monster.level}`,family,visualBlock,description,appearance,`behaviour: ${behaviour}`,`habitat: ${habitat}`,ecology?`ecology: ${ecology}`:'',quirk?`quirk: ${quirk}`:'',hook?`storytelling clue: ${hook}`:'',`show defining traits: ${traits}`,`clear visual manifestation of ${damage} power`,finish].filter(Boolean).join(', ')
}
...
function MonsterCard({ monster, onDelete, database=false, favorite=false, onFavorite, selected=false, onSelect }: { monster:Monster; onDelete?:()=>void; database?:boolean; favorite?:boolean; onFavorite?:()=>void; selected?:boolean; onSelect?:()=>void }) {
  ...
  return <article className={`card monsterCard${favorite ? ' favoriteCard' : ''}${selected ? ' selectedCard' : ''}`} data-db-record-id={database ? monster.id : undefined} data-db-record-kind={database ? 'monster' : undefined}>
    ...
    <div className="monsterIdentity"><span>LV <b>{monster.level}</b></span><span>{monster.rank}</span><span>{monster.species}</span><span>{style}</span></div>{monster.family && <div className="monsterFamilyBadge"><strong>{monster.family.familyName}</strong><span>{monster.family.form}</span><small>Family generation {monster.family.generation}</small></div>}
    ...
    {notes.length > 0 && <details className="monsterLore"><summary>{isAestraRecord?'Aestra lore & design notes':'Rules & design notes'}</summary><div className="lorePanel">{notes.map((n,i)=><p className="note" key={i}>{n}</p>)}</div></details>}
  </article>
}
...
function MonsterGenerator({ onSave }: { onSave:(m:Monster)=>void }) {
  ...
  const applyMonsterSetting=(monster:Monster):Monster=>{if(setting!=='Aestra')return monster;const regional=nation==='Aestra'?applyAestraCrystalInfluence(applyAestraWildIdentity(monster,environment,exposure,wildOrigin),influence):applyAestraMonsterIdentity(monster,nation,origin,influence,depth);return applyAestraRegionalVisualIdentity(regional,nation,{environment,exposure,origin:wildOrigin,influence,depth})};const make=()=>{const adjusted=powerAdjustedMonsterSettings(rank,complexity,soldierEquivalent,powerIntent);const regionalHint=setting==='Aestra'?(nation==='Aestra'?aestraWildGenerationHint(environment):aestraGenerationHint(nation)):null;const resolvedSpecies:Species=sp==='Auto'?(regionalHint?.species||species[Math.floor(Math.random()*species.length)]):sp;const hintedStyle=setting==='Aestra'&&combatStyle==='Mixed'?(regionalHint?.combatStyle||combatStyle):combatStyle;const pattern=inspiration==='Official Pattern'?officialInspiredMonsterSettings(readStored<Monster[]>('fu-monsters',[]),level,resolvedSpecies,adjusted.rank,hintedStyle,adjusted.complexity):{style:hintedStyle,complexity:adjusted.complexity,attackCount:adjusted.complexity==='Simple'?1:adjusted.complexity==='Standard'?2:3,skillCount:99,spellCount:99,affinityCount:99,sampleCount:0,note:''};const makeCandidate=()=>{let chassis=generateMonster({level,rank:adjusted.rank,soldierEquivalent:adjusted.soldierEquivalent,species:resolvedSpecies,complexity:pattern.complexity,combatStyle:pattern.style});if(inspiration==='Official Pattern')chassis=applyOfficialMonsterStructure(chassis,pattern);let monster=applyMonsterTheme(chassis,theme==='Auto'?undefined:theme);monster=applyMonsterSetting(monster);return {...monster,notes:[...(monster.notes||[]),...(setting==='Aestra'&&combatStyle==='Mixed'?[`Aestra generation hint: regional identity selected ${pattern.style} as the starting combat role because Combat Style was left Mixed.`]:[]),...(sp==='Auto'?[`Species selection: Auto chose ${resolvedSpecies}${setting==='Aestra'?' from the current Aestra context':' from the general species pool'}.`]:[]),`Power intent: ${powerIntent}.${powerIntent==='Legendary'?' Legendary intent promotes the generated chassis to Champion.':''}`,...(pattern.note?[pattern.note]:[])]}};const candidates=Array.from({length:6},()=>makeCandidate());const monster=candidates.reduce((best,candidate)=>validateMonsterDesign(candidate).score>validateMonsterDesign(best).score?candidate:best);setResult({...monster,notes:[...(monster.notes||[]).filter(line=>!line.startsWith('Quality selection: ')),`Quality selection: best structural result selected from ${candidates.length} generated candidates (${validateMonsterDesign(monster).score}/100).`]})}
  const reroll=(part:MonsterRerollPart)=>setResult(current=>{if(!current)return current;let next=rerollMonsterPart(current,part);if(setting==='Aestra'&&(part==='name'||part==='theme')&&nation!=='Aestra')next=refreshAestraMonsterLabel(next,nation);if(setting==='Aestra'&&part==='traits')next=nation==='Aestra'?refreshAestraWildTraits(next,environment):refreshAestraMonsterTraits(next,nation);if(setting==='Aestra'&&part==='affinities')next=nation==='Aestra'?refreshAestraWildAffinities(next,environment,influence):refreshAestraAffinities(next,nation,influence,depth);if(setting==='Aestra'&&(part==='visualIdentity'||part==='theme'))next=applyAestraRegionalVisualIdentity(next,nation,{environment,exposure,origin:wildOrigin,influence,depth});return next})
