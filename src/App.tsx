...
import { aestraNations, aestraOrigins, applyAestraMonsterIdentity, applyAestraCrystalInfluence, aestraGenerationHint, refreshAestraAffinities, refreshAestraMonsterLabel, AestraInfluence, AestraNation, AestraOrigin, valdoriaDepths, ValdoriaDepth } from './aestra'
...
function MonsterGenerator({ onSave }: { onSave:(m:Monster)=>void }) {
  ...
  const reroll=(part:MonsterRerollPart)=>setResult(current=>{if(!current)return current;let next=rerollMonsterPart(current,part);if(setting==='Aestra'&&(part==='name'||part==='theme'||part==='traits')&&nation!=='Aestra')next=refreshAestraMonsterLabel(next,nation);if(setting==='Aestra'&&part==='traits'&&nation==='Aestra')next=refreshAestraWildTraits(next,environment);if(setting==='Aestra'&&part==='affinities')next=nation==='Aestra'?refreshAestraWildAffinities(next,environment,influence):refreshAestraAffinities(next,nation,influence,depth);return next})
