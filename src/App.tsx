...
function buildMonsterArtPrompt(monster: Monster, preset: ArtPromptPreset, mode: ArtPromptMode, framing: ArtPromptFraming) {
  ...
  if (mode === 'Short') return [style,regional,frame,monster.name,identity,visualBlock,`traits: ${traits}`,promptSentence(appearance),`visible ${damage} power`,promptSentence(habitat),finish].filter(Boolean).join(', ')
  if (mode === 'Variant') return [style,regional,frame,`monster concept art of ${monster.name}`,identity,visualBlock,description,appearance,`behaviour: ${promptSentence(behaviour)}`,`ecology: ${promptSentence(ecology)}`,`memorable detail: ${promptSentence(quirk || hook)}`,`visible ${damage} energy`,finish].filter(Boolean).join(', ')
  if (mode === 'Permutation') return [style,regional,`${monster.name}, ${identity}`,visualBlock,'{full-body bestiary portrait, dramatic three-quarter pose, atmospheric environment portrait, dynamic battle-ready stance}',`traits: ${traits}`,promptSentence(appearance),`visible ${damage} power`,promptSentence(habitat),finish].filter(Boolean).join(', ')
  return [style,regional,frame,`bestiary illustration of ${monster.name}`,`${identity}, level ${monster.level}`,visualBlock,description,appearance,`behaviour: ${behaviour}`,`habitat: ${habitat}`,ecology?`ecology: ${ecology}`:'',quirk?`quirk: ${quirk}`:'',hook?`storytelling clue: ${hook}`:'',`show defining traits: ${traits}`,`clear visual manifestation of ${damage} power`,finish].filter(Boolean).join(', ')
}
...
function MonsterCard({ monster, onDelete, database=false, favorite=false, onFavorite, selected=false, onSelect }: { monster:Monster; onDelete?:()=>void; database?:boolean; favorite?:boolean; onFavorite?:()=>void; selected?:boolean; onSelect?:()=>void }) {
  ...
  return <article className={`card monsterCard${favorite ? ' favoriteCard' : ''}${selected ? ' selectedCard' : ''}`} data-db-record-id={database ? monster.id : undefined} data-db-record-kind={database ? 'monster' : undefined}>
    ...
    <div className="monsterIdentity"><span>LV <b>{monster.level}</b></span><span>{monster.rank}</span><span>{monster.species}</span><span>{style}</span></div>
