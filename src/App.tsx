...
function MonsterCard({ monster, onDelete, database=false, favorite=false, onFavorite }: { monster:Monster; onDelete?:()=>void; database?:boolean; favorite?:boolean; onFavorite?:()=>void }) {
  ...
  return <article className={`card monsterCard${favorite ? ' favoriteCard' : ''}`} data-db-record-id={database ? monster.id : undefined} data-db-record-kind={database ? 'monster' : undefined}>
...
function NativeDbActions({ kind, favoritesOnly, favoriteCount, count, onFavoritesOnly, onReset, onRandom, onCreate, onCopy, onExport, status }: { kind:DbKind; favoritesOnly:boolean; favoriteCount:number; count:number; onFavoritesOnly:()=>void; onReset:()=>void; onRandom:()=>void; onCreate:()=>void; onCopy:()=>void; onExport:()=>void; status:string }) {
...
function MonsterDatabase({ monsters,setMonsters,search,setSearch }: { monsters:Monster[]; setMonsters:React.Dispatch<React.SetStateAction<Monster[]>>; search:string; setSearch:(v:string)=>void }) {
  ...
  return <section>
    ...
    <Pagination page={safePage} total={totalPages} onPage={setPage}/>{filtered.length===0?<Empty text="No matching monsters saved yet."/>:<div className="grid">{paged.map(m=><MonsterCard key={m.id} monster={m} database favorite={favoriteMatches(favorites,'monster',m.id,m.name)} onFavorite={()=>setFavorites(current=>toggledFavorites(current,'monster',m.id,m.name))} onDelete={m.source==='Official'?undefined:()=>{if(window.confirm(`Delete “${m.name}”? This cannot be undone unless you have a backup.`))setMonsters(prev=>prev.filter(x=>x.id!==m.id))}}/>)}</div>}<Pagination page={safePage} total={totalPages} onPage={setPage}/>
  </section>
}

function MonsterGenerator({ onSave }: { onSave:(m:Monster)=>void }) {
  const initial=useMemo(()=>readStored(MONSTER_GENERATOR_KEY,{level:10,rank:'Soldier' as Rank,soldierEquivalent:3,sp:'Monster' as Species,complexity:'Standard' as 'Simple'|'Standard'|'Crunchy',combatStyle:'Mixed' as CombatStyle}),[])
  const [level,setLevel]=useState(initial.level),[rank,setRank]=useState<Rank>(initial.rank),[soldierEquivalent,setSoldierEquivalent]=useState(initial.soldierEquivalent),[sp,setSp]=useState<Species>(initial.sp),[complexity,setComplexity]=useState<'Simple'|'Standard'|'Crunchy'>(initial.complexity),[combatStyle,setCombatStyle]=useState<CombatStyle>(initial.combatStyle),[result,setResult]=useState<Monster|null>(null)
  useEffect(()=>localStorage.setItem(MONSTER_GENERATOR_KEY,JSON.stringify({level,rank,soldierEquivalent,sp,complexity,combatStyle})),[level,rank,soldierEquivalent,sp,complexity,combatStyle]);const make=()=>setResult(generateMonster({level,rank,soldierEquivalent,species:sp,complexity,combatStyle}))
...
function ItemDatabase({ items,setItems }: { items:AppItem[]; setItems:React.Dispatch<React.SetStateAction<AppItem[]>> }) {
  ...
  useEffect(()=>localStorage.setItem(ITEM_FILTERS_KEY,JSON.stringify({search,type,category,sourceFilter,martialFilter,materialFilter,sort,catalogFilter})),[search,type,category,sourceFilter,martialFilter,materialFilter,sort,catalogFilter]);useEffect(()=>localStorage.setItem(FAVORITES_KEY,JSON.stringify([...favorites])),[favorites]);useEffect(()=>{saveFavoritesOnly('item',favoritesOnly);setPage(1)},[favoritesOnly]);useEffect(()=>setPage(1),[search,type,category,sourceFilter,martialFilter,materialFilter,sort,catalogFilter])
  ...
  const summary=useMemo(()=>({weapons:filtered.filter(i=>i.type==='Weapon').length,armor:filtered.filter(i=>i.type==='Armor').length,shields:filtered.filter(i=>i.type==='Shield').length,accessories:filtered.filter(i=>i.type==='Accessory').length,official:filtered.filter(i=>i.source==='Official').length,custom:filtered.filter(i=>i.source!=='Official').length,martial:filtered.filter(i=>!!i.martial).length,materials:filtered.filter(i=>!!i.material).length,averageCost:filtered.length?Math.round(filtered.reduce((n,i)=>n+i.cost,0)/filtered.length):0}),[filtered]),favoriteCount=useMemo(()=>items.filter(i=>favoriteMatches(favorites,'item',i.id,i.name)).length,[items,favorites]),totalPages=Math.max(1,Math.ceil(filtered.length/DATABASE_PAGE_SIZE)),safePage=Math.min(page,totalPages),paged=useMemo(()=>filtered.slice((safePage-1)*DATABASE_PAGE_SIZE,safePage*DATABASE_PAGE_SIZE),[filtered,safePage])
