...
function ItemGenerator({ onSave }: { onSave:(item:AppItem)=>void }) {
  const initial=useMemo(()=>readStored(ITEM_GENERATOR_KEY,{type:'Weapon' as ItemType,weaponMethod:'Core Rare' as 'Core Rare'|'Atlas Custom',maxCost:1500,allowMartial:true,allowTransforming:true,damageType:'random' as DamageType|'random',addMaterial:false,materialNature:'Random' as MaterialNature|'Random',descriptorMode:'Random' as 'Elemental'|'Functional'|'Random',materialFunction:'Random' as MaterialFunction|'Random',itemTheme:'Auto' as ItemTheme|'Auto',powerIntent:'Standard' as GeneratorPowerIntent,inspiration:'Original' as 'Original'|'Official Pattern',setting:'Generic' as 'Generic'|'Aestra',nation:'Garlond' as AestraNation,origin:'Military' as AestraOrigin,influence:'Stable' as AestraInfluence}),[])
  const [type,setType]=useState<ItemType>(initial.type),[weaponMethod,setWeaponMethod]=useState<'Core Rare'|'Atlas Custom'>(initial.weaponMethod),[maxCost,setMaxCost]=useState(initial.maxCost),[allowMartial,setAllowMartial]=useState(initial.allowMartial),[allowTransforming,setAllowTransforming]=useState(initial.allowTransforming),[damageType,setDamageType]=useState<DamageType|'random'>(initial.damageType),[addMaterial,setAddMaterial]=useState(initial.addMaterial),[materialNature,setMaterialNature]=useState<MaterialNature|'Random'>(initial.materialNature),[descriptorMode,setDescriptorMode]=useState<'Elemental'|'Functional'|'Random'>(initial.descriptorMode),[materialFunction,setMaterialFunction]=useState<MaterialFunction|'Random'>(initial.materialFunction),[itemTheme,setItemTheme]=useState<ItemTheme|'Auto'>(initial.itemTheme||'Auto'),[powerIntent,setPowerIntent]=useState<GeneratorPowerIntent>(initial.powerIntent||'Standard'),[inspiration,setInspiration]=useState<'Original'|'Official Pattern'>(initial.inspiration||'Original'),[setting,setSetting]=useState<'Generic'|'Aestra'>(initial.setting||'Generic'),[nation,setNation]=useState<AestraNation>(initial.nation||'Garlond'),[origin,setOrigin]=useState<AestraOrigin>(initial.origin||'Military'),[influence,setInfluence]=useState<AestraInfluence>(initial.influence||'Stable'),[result,setResult]=useState<AppItem|null>(null)
