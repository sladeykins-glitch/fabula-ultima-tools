...
const visualSilhouettes:Record<Species,string[]>={
...
function detailedMonsterNotes(monster:Monster, theme:MonsterTheme, profile:ThemeProfile, primary:DamageType, status:string) {
  ...
  const identity=monster.visualIdentity
  const body=identity?.bodyPlan || pick(bodyPlans[monster.species])
  const surface=identity?.surface || pick(surfaces[theme])
...
export function rerollMonsterPart(monster:Monster, part:MonsterRerollPart):Monster {
  ...
  if(part==='theme') {
    ...
    return finishTheme({...monster,visualIdentity:undefined},nextTheme,primary,status,{name:true,attacks:true,skills:true,spells:true,traits:true,affinities:false})
  }
  const {primary,status}=gimmickFromMonster(monster,currentTheme)
