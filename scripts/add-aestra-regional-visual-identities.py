...
regional = Path('src/aestraVisualIdentity.ts')

regional.write_text(r'''import type { Monster, MonsterVisualIdentity } from './rules'
...
text = app.read_text()
import_marker = "import { itemQualitySummary, monsterQualitySummary, validateItemDesign, validateMonsterDesign } from './generatorQuality'\n"
new_import = "import { applyAestraRegionalVisualIdentity } from './aestraVisualIdentity'\n"
if new_import not in text:
    if import_marker not in text: raise SystemExit('App import marker not found')
    text = text.replace(import_marker, import_marker + new_import, 1)

old_apply = "const applyMonsterSetting=(monster:Monster):Monster=>setting!=='Aestra'?monster:(nation==='Aestra'?applyAestraCrystalInfluence(applyAestraWildIdentity(monster,environment,exposure,wildOrigin),influence):applyAestraMonsterIdentity(monster,nation,origin,influence,depth));"
new_apply = "const applyMonsterSetting=(monster:Monster):Monster=>{if(setting!=='Aestra')return monster;const regional=nation==='Aestra'?applyAestraCrystalInfluence(applyAestraWildIdentity(monster,environment,exposure,wildOrigin),influence):applyAestraMonsterIdentity(monster,nation,origin,influence,depth);return applyAestraRegionalVisualIdentity(regional,nation,{environment,exposure,origin:wildOrigin,influence,depth})};"
if old_apply not in text: raise SystemExit('applyMonsterSetting marker not found')
text = text.replace(old_apply, new_apply, 1)

old_reroll = "const reroll=(part:MonsterRerollPart)=>setResult(current=>{if(!current)return current;let next=rerollMonsterPart(current,part);if(setting==='Aestra'&&(part==='name'||part==='theme')&&nation!=='Aestra')next=refreshAestraMonsterLabel(next,nation);if(setting==='Aestra'&&part==='traits')next=nation==='Aestra'?refreshAestraWildTraits(next,environment):refreshAestraMonsterTraits(next,nation);if(setting==='Aestra'&&part==='affinities')next=nation==='Aestra'?refreshAestraWildAffinities(next,environment,influence):refreshAestraAffinities(next,nation,influence,depth);return next})"
new_reroll = "const reroll=(part:MonsterRerollPart)=>setResult(current=>{if(!current)return current;let next=rerollMonsterPart(current,part);if(setting==='Aestra'&&(part==='name'||part==='theme')&&nation!=='Aestra')next=refreshAestraMonsterLabel(next,nation);if(setting==='Aestra'&&part==='traits')next=nation==='Aestra'?refreshAestraWildTraits(next,environment):refreshAestraMonsterTraits(next,nation);if(setting==='Aestra'&&part==='affinities')next=nation==='Aestra'?refreshAestraWildAffinities(next,environment,influence):refreshAestraAffinities(next,nation,influence,depth);if(setting==='Aestra'&&(part==='visualIdentity'||part==='theme'))next=applyAestraRegionalVisualIdentity(next,nation,{environment,exposure,origin:wildOrigin,influence,depth});return next})"
if old_reroll not in text: raise SystemExit('reroll marker not found')
text = text.replace(old_reroll, new_reroll, 1)

old_hint = "<p className=\"muted smallText\">Targeted rerolls preserve level, rank, attributes, HP, MP, visual identity and Aestra regional identity. Visual Identity intentionally changes the creature's persistent appearance and its artwork prompt; Theme also creates a fresh visual identity.</p>"
new_hint = "<p className=\"muted smallText\">Targeted rerolls preserve level, rank, attributes, HP, MP, visual identity and Aestra regional identity. Visual Identity intentionally redesigns the creature using the current region's own silhouette, materials, palette, relic language and environmental motifs; Theme also creates a fresh regional visual identity.</p>"
if old_hint in text:
    text = text.replace(old_hint, new_hint, 1)

app.write_text(text)
print('Added Aestra regional visual identity system')
