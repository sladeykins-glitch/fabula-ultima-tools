...
evo=Path('src/generatorEvolution.ts')
...
if 'export interface MonsterFamilyIdentity' not in text:
    ...
    block="""export interface MonsterFamilyIdentity {\n  familyId: string\n  familyName: string\n  baseMonsterId: string\n  baseMonsterName: string\n  form: string\n  generation: number\n}\n\n"""
    if marker not in text: raise SystemExit('Monster marker missing')
    text=text.replace(marker,block+marker,1)
if 'family?: MonsterFamilyIdentity' not in text:
    marker='  visualIdentity?: MonsterVisualIdentity\n'
    if marker not in text: raise SystemExit('visualIdentity marker missing')
    text=text.replace(marker,marker+'  family?: MonsterFamilyIdentity\n',1)
rules.write_text(text)

# generatorEvolution.ts
text=evo.read_text()
...
function familyVisualIdentity(base:Monster,generated:Monster,variant:MonsterVariant):MonsterVisualIdentity|undefined{
  ...
  if(variant==='Mature')return {...next,scale:original.scale,signatureFeature:`a fully developed ${original.signatureFeature}`}
...
new='''function inheritFamilyIdentity(base:Monster,generated:Monster,variant:MonsterVariant):Monster{\n  const family=ensureFamilyIdentity(base)\n  const signature=signatureAttack(base)'''
if old not in text: raise SystemExit('inherit marker missing')
text=text.replace(old,new,1)
old_return="  return {...generated,traits:inheritedTraits,attacks,notes}\n}"
new_return="  const formName:Record<MonsterVariant,string>={Minion:'Lesser',Elite:'Elite',Champion:'Apex',Corrupted:'Corrupted',Elemental:'Elemental','Role Shift':'Role Shift',Juvenile:'Juvenile',Mature:'Mature',Elder:'Elder',Domesticated:'Domesticated','Regional Strain':'Regional Strain'}\n  const familyIdentity={...family,form:formName[variant],generation:family.generation+1}\n  return {...generated,traits:inheritedTraits,attacks,notes,visualIdentity:familyVisualIdentity(base,generated,variant),family:familyIdentity}\n}"
if old_return not in text: raise SystemExit('inherit return missing')
text=text.replace(old_return,new_return,1)

# Extend mechanics in createMonsterVariant
repls={
"  if(variant==='Minion'){rank='Soldier';level=clampLevel(base.level-5);soldierEquivalent=1}\n":"  if(variant==='Minion'){rank='Soldier';level=clampLevel(base.level-5);soldierEquivalent=1}\n  if(variant==='Juvenile'){rank='Soldier';level=clampLevel(base.level-10);soldierEquivalent=1}\n  if(variant==='Mature'){level=clampLevel(Math.max(base.level,base.level+5))}\n  if(variant==='Elder'){rank=base.rank==='Soldier'?'Elite':base.rank;level=clampLevel(base.level+10)}\n  if(variant==='Domesticated'){rank='Soldier';level=clampLevel(Math.max(5,base.level-5));style='Support'}\n  if(variant==='Regional Strain'){level=base.level}\n",
...
for a,b in repls.items():
    if a not in text: raise SystemExit('variant marker missing: '+a[:40])
    text=text.replace(a,b,1)
evo.write_text(text)

# App.tsx
text=app.read_text()
# Add family display on card
...
replace=marker+'{monster.family && <div className="monsterFamilyBadge"><strong>{monster.family.familyName}</strong><span>{monster.family.form}</span><small>Family generation {monster.family.generation}</small></div>}'
if marker not in text: raise SystemExit('monsterIdentity marker missing')
text=text.replace(marker,replace,1)
# Add family info into art prompt
needle="  const identity = `${monster.rank.toLowerCase()} ${theme.toLowerCase()} ${monster.species.toLowerCase()}`\n"
if needle not in text: raise SystemExit('art identity marker missing')
text=text.replace(needle,needle+"  const family = monster.family ? `creature family ${monster.family.familyName}, ${monster.family.form.toLowerCase()} form, visibly related to ${monster.family.baseMonsterName}` : ''\n",1)
for old,new in [
("[style,regional,frame,monster.name,identity,","[style,regional,frame,monster.name,identity,family,"),
("[style,regional,frame,`monster concept art of ${monster.name}`,identity,","[style,regional,frame,`monster concept art of ${monster.name}`,identity,family,"),
("[style,regional,`${monster.name}, ${identity}`,'{full-body","[style,regional,`${monster.name}, ${identity}`,family,'{full-body"),
("[style,regional,frame,`bestiary illustration of ${monster.name}`,`${identity}, level ${monster.level}`,","[style,regional,frame,`bestiary illustration of ${monster.name}`,`${identity}, level ${monster.level}`,family,")]:
    if old not in text: raise SystemExit('prompt list marker missing')
    text=text.replace(old,new,1)
# Add family variants UI after related variant buttons
